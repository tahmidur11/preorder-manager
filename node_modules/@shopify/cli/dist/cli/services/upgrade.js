...
export async function upgrade(directory, currentVersion) {
    ...
    if (projectDir) {
        ...
        throw new AbortError(output.content `Couldn't find the configuration file for ${output.token.path(directory)}, are you in a Shopify project directory?`);
    }
    else {
...
async function upgradeGlobalShopify(currentVersion) {
    ...
    const homebrewPackage = process.env.SHOPIFY_HOMEBREW_FORMULA;
    try {
...
async function upgradeGlobalViaHomebrew(homebrewPackage) {
    output.info(output.content `Homebrew installation detected. Attempting to upgrade via ${output.token.genericShellCommand('brew upgrade')}...`);
...
async function upgradeGlobalViaNpm() {
    ...
    output.info(output.content `Attempting to upgrade via ${output.token.genericShellCommand([command, ...args].join(' '))}...`);
