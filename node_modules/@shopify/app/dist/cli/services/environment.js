...
import { clearAppInfo, getAppInfo, setAppInfo } from './conf.js';
...
export async function ensureDevEnvironment(options, token) {
    ...
    if (!organization)
        throw new BugError(`Couldn't find Organization with id ${orgId}.`);
    if (!selectedApp) {
        if (cachedInfo?.appId) {
            const app = await fetchAppFromApiKey(cachedInfo.appId, token);
            if (!app)
                throw new BugError(`Couldn't find App with apiKey ${cachedInfo.appId}.`);
            selectedApp = app;
        }
        else {
...
function buildOutput(app, store, cachedInfo) {
    return {
        remoteApp: {
            ...app,
...
export async function fetchAppAndIdentifiers(options, token) {
    ...
    if (options.reset) {
        ...
        if (!partnersApp) {
            throw new AbortError(output.content `Couldn't find the app with API key ${apiKey}`, output.content `• If you didn't intend to select this app, run ${output.content `${output.token.packagejsonScript(options.app.packageManager, 'deploy', '--reset')}`.value}`);
        }
    }
    else {
...
async function fetchDevDataFromOptions(options, orgId, token) {
    ...
    if (options.storeFqdn) {
        ...
        if (!orgWithStore.store) {
            ...
            throw new BugError(`Could not find ${options.storeFqdn} in the Organization ${org.businessName} as a valid development store.`, `Visit https://${partners}/${org.id}/stores to create a new store in your organization`);
        }
