...
export async function setupBundlerAndFileWatcher(options) {
    ...
    options.devOptions.extensions.forEach(async (extension) => {
        bundlers.push(bundleExtension({
            ...
            watch: (error) => {
                output.debug(`The Javascript bundle of the UI extension with ID ${extension.devUUID} has ${error ? 'an error' : 'changed'}`, error ? options.devOptions.stderr : options.devOptions.stdout);
        ...
        abortController.signal.addEventListener('abort', () => {
            ...
            localeWatcher
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                .catch((error) => {
                output.debug(`Locale file watching failed to close for extension with ${extension.devUUID}: ${error.message}`, options.devOptions.stderr);
            });
        });
    });
