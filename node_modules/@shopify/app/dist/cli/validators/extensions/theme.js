...
function validateFile(filepath, dirname) {
    if (!SUPPORTED_BUCKETS.includes(dirname)) {
        throw new AbortError(output.content `Your theme app extension includes files in an unsupported directory, ${output.token.path(dirname)}`, `Make sure all theme app extension files are in the supported directories: ${SUPPORTED_BUCKETS.join(', ')}`);
    }
