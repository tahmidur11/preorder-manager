import { AppConfigurationSchema, WebConfigurationSchema, App, WebType } from './app.js';
import { configurationFileNames, dotEnvFileNames } from '../../constants.js';
import metadata from '../../metadata.js';
import { UIExtensionInstance } from '../extensions/ui.js';
...
import { getDependencies, getPackageManager, getPackageName, usesWorkspaces as appUsesWorkspaces, } from '@shopify/cli-kit/node/node-package-manager';
...
class AppLoader {
    constructor({ directory, mode, specifications }) {
        this.appDirectory = '';
    ...
    async loaded() {
        ...
        const { functions, usedCustomLayout: usedCustomLayoutForFunctionExtensions } = await this.loadFunctions(configuration.extensionDirectories);
        const { uiExtensions, usedCustomLayout: usedCustomLayoutForUIExtensions } = await this.loadUIExtensions(configuration.extensionDirectories);
        const { themeExtensions, usedCustomLayout: usedCustomLayoutForThemeExtensions } = await this.loadThemeExtensions(configuration.extensionDirectories);
        ...
        const appClass = new App(name, 'SHOPIFY_API_KEY', this.appDirectory, packageManager, configuration, configurationPath, nodeDependencies, webs, uiExtensions, themeExtensions, functions, usesWorkspaces, dotenv);
        if (!this.errors.isEmpty())
            appClass.errors = this.errors;
    ...
    async getConfigurationPath() {
        ...
        if (!configurationPath) {
            throw new AbortError(output.content `Couldn't find the configuration file for ${output.token.path(this.directory)}, are you in an app directory?`);
        }
        this.configurationPath = configurationPath;
        return configurationPath;
    }
    async loadWebs(webDirectories) {
    ...
    async parseConfigurationFile(schema, filepath, 
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    decode = decodeToml) {
    ...
    async loadUIExtensions(extensionDirectories) {
        ...
        const extensions = configPaths.map(async (configurationPath) => {
            ...
            if (!specification) {
                ...
                this.abortOrReport(output.content `Unknown extension type ${output.token.yellow(type)} in ${output.token.path(configurationPath)}. ${isShopifolk ? shopifolkMessage : ''}`, undefined, configurationPath);
                return undefined;
            }
            ...
            if (specification.singleEntryPath) {
                entryPath = (await Promise.all(['index']
                    ...
                    .map(async (sourcePath) => ((await fileExists(sourcePath)) ? sourcePath : undefined)))).find((sourcePath) => sourcePath !== undefined);
                if (!entryPath) {
                    this.abortOrReport(output.content `Couldn't find an index.{js,jsx,ts,tsx} file in the directories ${output.token.path(directory)} or ${output.token.path(joinPath(directory, 'src'))}`, undefined, directory);
                }
            }
    ...
    async loadFunctions(extensionDirectories) {
        ...
        const allFunctions = configPaths.map(async (configurationPath) => {
            ...
            const specification = this.findSpecificationForType(type);
            if (!specification) {
                this.abortOrReport(output.content `Unknown function type ${output.token.yellow(type)} in ${output.token.path(configurationPath)}`, undefined, configurationPath);
                return undefined;
            }
    ...
    async loadThemeExtensions(extensionDirectories) {
        ...
        const extensions = configPaths.map(async (configurationPath) => {
            ...
            const specification = this.findSpecificationForType('theme');
            if (!specification) {
                this.abortOrReport(output.content `Unknown theme type ${output.token.yellow('theme')} in ${output.token.path(configurationPath)}`, undefined, configurationPath);
                return undefined;
            }
...
export async function loadAppName(appDirectory) {
...
async function logMetadataForLoadedApp(app, loadingStrategy) {
    await metadata.addPublicMetadata(async () => {
        ...
        const webBackendFramework = webBackendCount === 1
        ...
        const allExtensions = [...app.extensions.function, ...app.extensions.theme, ...app.extensions.ui];
        const extensionsBreakdownMapping = {};
        ...
        return {
            ...
            app_extensions_custom_layout: loadingStrategy.usedCustomLayoutForFunctionExtensions ||
            ...
            app_scopes: JSON.stringify(app.configuration.scopes
                .split(',')
