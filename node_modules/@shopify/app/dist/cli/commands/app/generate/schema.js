...
export default class GenerateSchema extends Command {
    async run() {
        ...
        if (!extension) {
            ...
            throw new AbortError(output.content `No function named ${args.function} found in this app.`, output.content `Use one of the available functions: ${functions}`);
        }
        output.info(await generateSchemaService({ app, extension, apiKey }));
    }
}
GenerateSchema.description = 'Generates a GraphQL schema for a function.';
GenerateSchema.flags = {
    ...globalFlags,
    ...appFlags,
...
GenerateSchema.args = [{ name: 'function', required: true, description: 'The name of the function to fetch the schema for.' }];
